...
export default function SafetyNetDisplay() {
  const { setCalculations, calculations } = useCalculatorStore()

  useEffect(() => {
    // Only calculate if we haven't already set safety net amount
    if (!calculations.safetyNetAmount) {
      // Dummy calculation for safety net (lifetime income + age pension)
      const lifetimeIncome = 18500 // Example amount
      const agePension = calculations.estimatedPension || 28976
      const safetyNet = lifetimeIncome + agePension
      ...
      >
        ...
        >
          <p className="text-lg text-foreground">
            ✨ This income is guaranteed whether you live to 90, 100, or beyond
          </p>
        </motion.div>
      </motion.div>
    </div>
